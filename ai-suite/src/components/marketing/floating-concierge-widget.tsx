...
export function FloatingConciergeWidget() {
  ...
  const handleOpenTool = React.useCallback(
    (tool: ToolName, opts?: { draftText?: string; scroll?: boolean }) => {
      ...
      if (pathname === "/") {
        router.replace(target, { scroll: opts?.scroll ?? true });
      } else {
        router.push(target);
      }
    },
  ...
  if (!mounted || hideOnHome) return null;
  ...
  return createPortal(
    ...
    >
      ...
      <button
        ...
        className={cn(
          interactiveClick,
          "pointer-events-auto ml-auto flex items-center gap-2 rounded-full border py-2 pl-2 pr-3 shadow-lg backdrop-blur-xl",
