/*
  ...
  1. New Tables
    - `einvoice_details` - e-Invoice/e-Archive details with GİB integration
    - `tax_rates` - Tax rate definitions and rules
    - `einvoice_queue` - Queue for sending invoices to GİB
    - `einvoice_logs` - Operation logs

  2. Customer Extensions
    - Add tax-related fields to customers table
...
CREATE TABLE IF NOT EXISTS public.einvoice_details (
    ...
    gib_error_code TEXT,
    gib_error_detail TEXT,
    ai_validation_score DECIMAL(3,2),
    ai_validation_issues JSONB,
    tax_anomaly_detected BOOLEAN DEFAULT false,
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'tax_number') THEN
        ALTER TABLE public.customers ADD COLUMN tax_number TEXT;
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'tax_id_type') THEN
        ALTER TABLE public.customers ADD COLUMN tax_id_type TEXT CHECK (tax_id_type IN ('vkn', 'tckn', 'foreign'));
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'is_tax_exempt') THEN
        ALTER TABLE public.customers ADD COLUMN is_tax_exempt BOOLEAN DEFAULT false;
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'exemption_reason') THEN
        ALTER TABLE public.customers ADD COLUMN exemption_reason TEXT;
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'gib_registered') THEN
        ALTER TABLE public.customers ADD COLUMN gib_registered BOOLEAN DEFAULT false;
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'einvoice_address') THEN
        ALTER TABLE public.customers ADD COLUMN einvoice_address TEXT;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_customers_tax_number ON public.customers(tax_number);
...
CREATE POLICY "Tenant isolation" ON public.einvoice_details FOR ALL TO authenticated
USING (true) WITH CHECK (true);

CREATE POLICY "Tenant isolation" ON public.tax_rates FOR ALL TO authenticated
USING (true) WITH CHECK (true);

CREATE POLICY "Tenant isolation" ON public.einvoice_queue FOR ALL TO authenticated
USING (true) WITH CHECK (true);

CREATE POLICY "Tenant isolation" ON public.einvoice_logs FOR ALL TO authenticated
USING (true) WITH CHECK (true);

-- =====================================================
-- 7. DEFAULT TAX RATES (Turkey)
-- =====================================================

INSERT INTO public.tax_rates (tenant_id, tax_type, tax_name, tax_rate, applies_to, valid_from, gib_tax_code, is_active)
VALUES
    ('00000000-0000-0000-0000-000000000000', 'kdv', 'KDV %20', 20.00, 'all', '2024-01-01', '0015', true),
