/*
  ...
  1. New Tables
    - `cash_flow_rules` - Dynamic prediction rules
    - `cash_flow_predictions` - ML prediction results
    - `ai_model_metrics` - Model performance tracking
    - `production_bom` - Bill of Materials for production

  2. Security
    - RLS enabled on all tables
    - Tenant-based access control
    - Service role for ML operations

  3. Features
    - Multi-scenario predictions (pessimistic/realistic/optimistic)
...
CREATE POLICY "Users can insert cash flow rules"
  ON public.cash_flow_rules FOR INSERT
  TO authenticated
  WITH CHECK (tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid);

CREATE POLICY "Users can update own tenant cash flow rules"
  ON public.cash_flow_rules FOR UPDATE
  TO authenticated
  USING (tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid)
  WITH CHECK (tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid);

-- =====================================================
-- 2. CASH FLOW PREDICTIONS TABLE
-- =====================================================
...
CREATE POLICY "Service role can manage predictions"
  ON public.cash_flow_predictions FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- =====================================================
-- 3. AI MODEL METRICS TABLE
-- =====================================================
...
-- =====================================================
-- 4. PRODUCTION BOM (BILL OF MATERIALS)
-- =====================================================
...
CREATE POLICY "Users can manage own tenant BOM"
  ON public.production_bom FOR ALL
  TO authenticated
  USING (tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid)
  WITH CHECK (tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid);
...
-- Calculate total BOM cost for a product
...
BEGIN
  SELECT COALESCE(SUM(
    bom.quantity * COALESCE(bom.cost_per_unit, p.purchase_price, 0) * (1 + bom.waste_percentage / 100)
  ), 0)
  INTO v_total_cost
  FROM public.production_bom bom
  LEFT JOIN public.products p ON bom.component_product_id = p.id
...
-- Get prediction accuracy for tenant/branch
CREATE OR REPLACE FUNCTION public.get_prediction_accuracy(
  p_tenant_id uuid, 
...
RETURNS TABLE(
  ...
  low_risk_days bigint,
  high_risk_days bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND prediction_date >= CURRENT_DATE - p_days
    AND actual_balance IS NOT NULL
    AND scenario_type = 'realistic';
END;
$$;